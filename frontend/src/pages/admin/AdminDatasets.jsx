...
function extractApiError(err, fallback) {
  ...
  if (Array.isArray(apiData?.errors) && apiData.errors.length > 0) {
    return apiData.errors[0]?.msg || fallback;
  }
  return apiData?.message || err?.message || fallback;
}
...
export default function AdminDatasets() {
  ...
  const loadRows = async () => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message || `Failed to load ${activeMeta.label}.`,
      );
      setRows([]);
    } finally {
      setLoading(false);
  ...
  const formatRemaining = (dateValue) => {
    if (!dateValue) return "-";
  ...
  const buildPayload = () => {
    if (dataset === "countries") {
      return {
        ...
        destructionDate: form.destructionDate
          ? new Date(form.destructionDate)
          : null,
      };
    }
  ...
  const startEdit = (row) => {
    ...
    if (dataset === "countries") {
      setForm({
        ...
        destructionDate: toIsoDateInput(row.destructionDate),
      });
      return;
    }
    if (dataset === "commodities") {
  ...
  const nextDestructionAt = useMemo(() => {
    if (dataset !== "countries") return null;
    const now = Date.now();
    const timestamps = rows
      .map((r) =>
        r.destructionDate ? new Date(r.destructionDate).getTime() : NaN,
      )
  ...
  const removeRow = async (id) => {
    ...
    setError("");
    try {
      await axios.delete(`${API}/${activeMeta.path}/${id}`, {
        headers: authHeaders,
      });
      await loadRows();
    ...
    <div className="space-y-4">
      ...
      {dataset === "countries" && (
        <div className="bg-[#121212] border border-[#2a2a2a] rounded-2xl p-4">
          <p className="text-sm text-neutral-400">Destruction date</p>
          <p className="text-xl text-neutral-100 font-semibold">
            {nextDestructionAt ? nextDestructionAt.toLocaleString() : "N/A"}
          </p>
        </div>
      )}
      ...
      <form
        onSubmit={submit}
        className="bg-[#121212] border border-[#2a2a2a] rounded-2xl p-4 space-y-3"
      >
        ...
        {dataset === "countries" && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <input
              className="bg-[#171717] border border-[#2a2a2a] rounded-xl px-3 py-2 text-neutral-100"
              placeholder="Code (2-3 letters, e.g. TH)"
              value={form.code}
            ...
            <input
              ...
              placeholder="GDP"
              value={form.GDP}
              onChange={(e) => onChange("GDP", e.target.value)}
            />
            <input
              type="number"
              step="any"
            ...
            <input
              ...
              value={form.tradeBalance}
              onChange={(e) => onChange("tradeBalance", e.target.value)}
            />
            <input
              type="date"
              className="bg-[#171717] border border-[#2a2a2a] rounded-xl px-3 py-2 text-neutral-100"
              ...
              onChange={(e) => onChange("destructionDate", e.target.value)}
            />
          </div>
        )}
        ...
        {dataset === "trade" && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <input
              type="date"
              className="bg-[#171717] border border-[#2a2a2a] rounded-xl px-3 py-2 text-neutral-100 md:col-span-2"
              value={form.date}
      ...
      <div className="bg-[#121212] border border-[#2a2a2a] rounded-2xl overflow-x-auto">
        {loading ? (
          <p className="text-neutral-400 p-6">
          ...
          <table className="w-full text-sm">
            <thead className="table-head">
              {dataset === "countries" && (
                <tr>
                  ...
                  <th className="px-4 py-3 text-left">Destruction Date</th>
                  <th className="px-4 py-3 text-left">Remaining Time</th>
            ...
            <tbody className="divide-y divide-[#2a2a2a]">
                ...
                <tr key={row._id}>
                  {dataset === "countries" && (
                    <>
                      ...
                      <td className="px-4 py-3 text-neutral-300">
                        {row.destructionDate
                          ? toIsoDateInput(row.destructionDate)
                          : "-"}
                      </td>
                      <td className="px-4 py-3 text-neutral-300">
                        {formatRemaining(row.destructionDate)}
                      </td>
                    </>
                  )}
                  {dataset === "commodities" && (
                    <>
                      ...
                      <td className="px-4 py-3 text-neutral-300">
                        {row.currentPrice != null
                          ? Number(row.currentPrice).toLocaleString()
                          : "-"}
                      </td>
                    </>
                  )}
              ...
              {rows.length === 0 && (
                <tr>
                  <td
                    colSpan={dataset === "trade" ? 6 : dataset === "countries" ? 6 : 5}

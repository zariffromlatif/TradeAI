...
- **Silver**:
  - Core dashboard/compare/commodity/trade views
  - Basic authenticated features
  - More limited advanced analytics/reporting access
- **Gold**:
  - Advanced analytics routes (including gated endpoints like batch risk and report capabilities)
  - Larger forecast horizon cap
- **Diamond**:
  - Highest feature limits (largest forecast horizon cap)
...
- user sees a clear fallback notice
...
2. Copy `backend/.env.example` -> `backend/.env` and fill required values
3. Start services:
...
Reports use a queue-first model. If queue/Redis is unavailable, the backend now falls back to inline generation to keep report creation functional.

This means:

- report generation does not hard-fail only because queue infra is down
- admin metrics expose queue availability
...
- Forecast/risk integration via analytics routes + ML proxying
- Payments: `/api/payment/*`
- Reports: `/api/reports/*`
- Marketplace/orders/simulation/advisory APIs under their respective route prefixes

---

## Deployment Notes

- Frontend can be deployed on Vercel/Netlify/static hosting.
- Backend and ML service can be deployed separately (Render/Railway/etc.).
- Ensure backend can reach ML service URL in deployed environment.
- Ensure CORS and `FRONTEND_PUBLIC_URL` are correct.
- Use production secrets and disable demo payment in production.

---

## Troubleshooting

### Compare page shows no selected-commodity overlap

- Expected for sparse commodity-country pairs with current data coverage.
...
- With latest backend routes, inline fallback should still generate reports.
...
Internal academic/project repository unless otherwise specified by the project owner.
